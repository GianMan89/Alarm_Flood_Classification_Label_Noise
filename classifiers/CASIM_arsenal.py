...
kernel based ensemble of MULTIROCKET classifiers.
"""

__author__ = ["MatthewMiddlehurst", "kachayev", "GianlucaManca"]
__all__ = ["Arsenal"]
...
from .CASIM_multirocket import MultiRocketMultivariate
...
class Arsenal(BaseClassifier):
    ...
    def __init__(
        ...
        contract_max_n_estimators=1000,
        ...
        self.n_jobs = n_jobs
        self.n_jobs_multirocket = n_jobs_multirocket
        self.alphas = alphas
        self._threads_to_use = self.n_jobs
    ...
    def _fit(self, X, y):
        ...
        base_rocket = MultiRocketMultivariate(
            ...
            n_jobs=self.n_jobs_multirocket,
        )
        ...
        self.estimator_weights = np.array(self.weights_) / self._weight_sum
    ...
    def _fit_predict_proba(self, X, y):
        ...
        if time_limit > 0:
            ...
            while (
                ...
                fit = Parallel(n_jobs=self._threads_to_use)(
                    delayed(self._fit_predict_proba_estimator)(
                        _clone_estimator(
                            base_rocket,
                            None
                            if self.random_state is None
                ...
                y_proba, estimators, transformed_data = zip(*fit)
        ...
        return np.around(
            np.sum(y_probas, axis=0) / self.n_estimators,
            8,
        )
    ...
    def _predict_proba(self, X) -> np.ndarray:
        ...
        return np.around(
            np.sum(y_probas, axis=0) / (np.ones(self.n_classes_) * self._weight_sum),
            8,
        )
    ...
    def _fit_estimator(self, rocket, X, y):
        ...
        transformed_x = scaler.fit_transform(transformed_x, y)
        ...
        return [
            make_pipeline(rocket, scaler, clf),
            transformed_x if self.save_transformed_data else None,
        ]

    def _fit_predict_proba_estimator(self, rocket, X, y):
        transformed_x = rocket.fit_transform(X)
        pipelines = []
        y_pred_proba = []
        scaler = StandardScaler(with_mean=False)
        transformed_x = scaler.fit_transform(transformed_x, y)
        clf = CalibratedClassifierCV(
            base_estimator=RidgeClassifierCV(
                alphas=self.alphas,
            ),
        )
        # transformed_x = np.nan_to_num(transformed_x, nan=0.0, posinf=0.0, neginf=0.0)
        clf.fit(transformed_x, y)
        y_scores = clf.predict_proba(transformed_x)
        return [
            np.array(y_scores),
            make_pipeline(rocket, scaler, clf),
            transformed_x if self.save_transformed_data else None,
    ...
    def _predict_proba_for_estimator(self, X, classifier, idx):
        ...
        for i in range(0, X.shape[0]):
            ...
            weights[i] += self.weights_[idx] * preds[i]
        return weights
    ...
    def _train_probas_for_estimator(self, y, idx):
        ...
        preds = clf.predict_proba(self.transformed_data_[idx].iloc[oob])

        weight = 1

        for n, pred in enumerate(preds):
            # results[oob[n]][self._class_dictionary[pred]] += weight
            results[oob[n]] += weight * pred

        return results, weight, oob

    def get_attributes(self):
        n_estimators = self.n_estimators
        estimator_weights = list(self.estimator_weights)
        n_features = []
        for i in range(n_estimators):
            n_features.append(self.estimators_[i].steps[0][1].num_features)
        return (
            n_estimators,
            n_features,
            estimator_weights,
